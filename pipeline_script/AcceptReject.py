...
header = "Motif_cid\tIsBadMotif\tPositionBiasZscore\tPeakRankBiasZscore\tGlobalEnrichmentZscore\tCombineAvgScore\tEnrichmentRank(total:" + str(
    maxRank) + ")\tbestMotif"
outf.write(header + "\n")
tfLogOdd = dict()
tfBestScore = dict()
...
for j in range(len(chipseq_motif) - 1):
    ...
    for motif in toks[0:(len(toks) - 1)]:
        ...
        r = int(maxRank - rank_vec[scoreCol])
        outf.write(m + "\t" + isbad + "\t" + Arr2str(score_vec) +
                   "\t" + str(r + 1) + "\t" + bestmotif + "\n")
...
for tf in tfBestScore:
    ...
    if tfBestMotif[tf] not in modelparameter_z1:
        probstr += "!"
    outf.write(Arr2str([tf, float(logOdds), probstr]) +
               "\t" +
               tfBestMotif[tf] +
               "\t" +
               (tfBestMotifScore[tf]) +
               "\t" +
               tf_validatability[tf] +
               "\n")
outf.close()
