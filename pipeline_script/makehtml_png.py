...
username = sys.argv[3]
dataPath = root + "/data"
personaldir = "https://personal.broadinstitute.org/" + username
dataUrl = personaldir + "/motifpipeline/data/"
...
def imageHtml(image_src, title):
    ret_str = image_src.replace(
        "/home/unix/" + username + "/public_html",
        personaldir + "").replace(
            "/broad/hptmp/" + username + "/motifpipeline",
            personaldir + "/motifpipeline/jobdata")
    ret_str = "<div  style=\"width:218px; overflow-x:hidden;\"><embed src=" + \
        ret_str + " width=218px height=200px  onload=lzld(this)></embed>" + \
        title + "</div>\n"
    return ret_str
...
def motifPosRankPlot(bestmotif):
    ...
    if not os.path.isfile(outfn):
        cmd = "Rscript " + root + "/plotMotifPosRankDistr.R " + \
            olFile + " " + bestmotif + " " + resultDir + "/plots/"
        oss(cmd)
    ret_str = outfn.replace(
        "/home/unix/" + username + "/public_html",
        personaldir + "").replace(
        "/broad/hptmp/" + username + "/motifpipeline",
        personaldir + "/motifpipeline/jobdata")
    ret_str = "<div  style=\"width:420px; overflow-x:hidden;\"><embed src=" + \
        ret_str + " width=420px height=200px onload=lzld(this)></embed>" + \
        "Motif Position and Peak-Rank Distribution</div>\n"
    return ret_str
...
def MotifClustRow(motifclust):
    ...
    if os.path.isfile(imageFn):
        negbinomModel = "<td colspan=2>" + \
            imageHtml(imageFn.replace(dataPath, dataUrl),
                      "Trained Model for Enrichment Rank") + "</td>"
    else:
...
def motifLogo(motif):
    logourl = personaldir + "/logos2/" + \
        motif + ".png"
    ret_str = "<div  style=\"width:218px; overflow-x:hidden;\"><embed  src=" + \
        logourl + \
        " width=218px  height=100px  onload=lzld(this)></embed>best motif: " + \
        motif + "</div>\n"
    return ret_str


def TFTable(lines):
    header = "TFName AcceptLogOdds AcceptProb BestMotifClusterId " + \
        "PositionBiasZscore PeakRankBiasZscore GlobalEnrichmentZscore " + \
        "CombineAvgScore RejectCapability AcceptCapability"
    ...
    for line in lines:
        ...
        outstr += TFImageTRs(comps[0], comps[3])
...
def MotifClustTable(lines):
    header = "MotifClusterId IsWrongMotif PositionBiasZscore " + \
        "PeakRankBiasZscore GlobalEnrichmentZscore CombineAvgScore " + \
        "EnrichmentRank(total:283) bestMotif"
...
htmlContent = ""
CSSstr = " <style type=\"text/css\">img[alt=small] { float: center; " + \
    "width: 300px;}img[alt=large]{float: center; width: 800px;}</style>"
htmlContent += CSSstr
...
htmlContent += "<body>"
htmlContent += " <script src=\"" + personaldir + \
    "/motifpipeline/data/lazyload.min.js\"></script>"
# ####make summary page #####
