...
for line in open(root + "/data/motifs-clust-names.txt"):
    ...
    for m in motifset:
        ...
        if m not in motifsWinstance:
            continue
        # mean there is exist cluster contain it
        if tfname in motifNames_clust and motifNames_clust[tfname] != cid:
            # print tfname
...
def getRankandLabel(fn):
    ...
    for line in lines:
        ...
        if len(cid_name[motifname_cid]
               ) < 2:  # filter motif with name only one character
            continue
  ...
  # make sure only count the best match motif group in each dataset
    ...
    ##
    return (rank_mat, Y[0:i], motifNameList,
            Zscore_mat[0:i, :], mncid_bestmotif)
...
for line in gzip.open(GlobalEnrichmentFn):
    ...
    if comps[2] == "+":
        ...
        if cid not in dataset_motifscore[d]:
            ...
            dataset_motifscore[d][cid] = max(
                float(score), dataset_motifscore[d][cid])
...
bname = os.path.basename(fl).replace(
    ".pickle",
    "").replace(
        ".score",
        "").replace(
...
score_mat = np.column_stack(
    (score_mat, np.zeros((score_mat.shape[0], 2))))  # score matrix
gbcol = 2
...
score_mat[:, avgcombin_col] = scipy.stats.mstats.zscore(
    score_mat[:, gbcol] + score_mat[:, 0] + score_mat[:, 1])
X[:, avgcombin_col] = scipy.stats.rankdata(score_mat[:, avgcombin_col])
for i in range(len(motifnamelist)):
    oline = bname + "\t" + cid_name[motifnamelist[i]
                                    ] + "\t" + "\t".join(map(str, score_mat[i, :]))
    outf.write(oline + "\n")
...
pickle.dump([combined_Y,
             combined_rank_mat,
             combined_chipseqmotif,
             combined_zscore_mat,
             mncid_bestmotif],
            open(PosRankEnrichmentFn + ".summary.pickle",
                 "wb"))
