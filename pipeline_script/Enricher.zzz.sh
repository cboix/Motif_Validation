...
if [ ! -f $DIR/regions.txt.gz ]; then 
       echo "Making $DIR/regions.txt.gz from $INREGIONS" 
       echo "$REGIONLIST is regions"
	if [ ! -z "$INREGIONS" ]; then
		gunzip -cf $INREGIONS
	else
		cat
...
for r in $(echo $BACKREGIONS); do 
	...
	if [ ! -f $DIR/compare/$(rname $r)/regions.txt.gz ]; then 
        ...
        echo "Using $DIR/regions.txt.gz"
        echo "With $DIR/compare/$(rname $r)/bregions.txt.gz"
        echo "Merge with $DIR/compare/$(rname $r)/bregions.txt.gz"
        echo "Send to $DIR/compare/$(rname $r)/regions.txt.gz"
        echo "$HSTR is whether to stick to one strand or not"
        # NOTE: Currently throws a gzip stdout broken pipe error but still works.
		rtool flatten -s $HSTR $DIR/regions.txt.gz | rtool crop -s $HSTR - $DIR/compare/$(rname $r)/bregions.txt.gz | rtool merge - $DIR/compare/$(rname $r)/bregions.txt.gz | gzip -c > $DIR/compare/$(rname $r)/regions.txt.gz; echo "Done"
	fi

    if [ ! -f $DIR/compare/$(rname $r)/regions_sizes.txt ]; then 
        echo "Counting region sizes"
        # 1) region
        ...
        }'  - > $DIR/compare/$(rname $r)/regions_sizes.txt; echo "Done"
	fi
done
...
if [ ! -z "$MOTIFFILE" ]; then
    echo "Running Motif instance pipeline"
	OPTMMDIR=$DIR/optmm
	KMERSD=X
...
if [ "$LOWMODE" = 0 ]; then
    ...
    echo "Done with counting overlap"
else
