...
def imageHtml(image_src, title):
    ret_str = image_src.replace(
        "/web/personal/" + username + "",
        personaldir + "").replace(
            "/broad/hptmp/" + username + "/motifpipeline",
            personaldir +
            "/motifpipeline/jobdata")
...
def motifPosRankPlot(bestmotif):
    ...
    if not os.path.isfile(outfn):
        cmd = "Rscript " + root + "/plotMotifPosRankDistr.R " + olFile + " " + \
            bestmotif + " " + resultDir + "/plots/"
        oss(cmd)
...
def MotifClustRow(motifclust):
    ...
    if os.path.isfile(imageFn):
        negbinomModel = "<td colspan=2>" + imageHtml(imageFn.replace(
            dataPath, dataUrl), "Trained Model for Enrichment Rank") + "</td>"
    else:
        negbinomModel = "<td colspan=2>" + "No Training Data" + "</td>"
        enrichRank = writeTag("td", "Enrichment Rank:<br>" +
...
def motifLogo(motif):
    logourl = personaldir + "/logos_pdf/" + motif + ".big.pdf"
    ret_str = "<div  style=\"width:218px; overflow-x:hidden;\"><embed  src=" + \
        logourl + \
        " width=218px  height=200px  onload=lzld(this)></embed>best motif: " + \
        motif + "</div>\n"
    return ret_str
